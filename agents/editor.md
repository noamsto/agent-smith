...
5. **Revision pass** — if HEAD already contains your earlier edit and you are now
   ...
   delta. Ground it on `git -C "$repo_root" diff <base>...HEAD` (the cumulative
   diff) before writing the summary.
...

{
  ...
  "summary": "<imperative one-line description of the CUMULATIVE change vs the base branch (what the whole PR introduces), used as the PR/commit subject — on a revision pass describe the full change, NOT the revision delta — NO conventional-commit type prefix (no `feat:`/`chore:`); the applier prepends one from fix_type>",
  "reason": ""
}
